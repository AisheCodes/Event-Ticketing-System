...
function initTheme() {
    ...
    // Apply theme by updating CSS variables so the colours actually change
    function applyTheme(theme) {
        const root = document.documentElement;
        root.setAttribute('data-theme', theme);
        if (theme === 'light') {
            // Light mode variable overrides
            root.style.setProperty('--navy', '#f5f7fb');
            root.style.setProperty('--navy-light', '#ffffff');
            root.style.setProperty('--navy-lighter', '#e6eef8');
            root.style.setProperty('--white', '#0a192f');
            root.style.setProperty('--glass-bg', 'rgba(255,255,255,0.8)');
            root.style.setProperty('--glass-border', 'rgba(10,25,47,0.06)');
            root.style.setProperty('--yellow', '#c08b00');
            root.style.setProperty('--yellow-dark', '#a36f00');
        } else {
            // Dark mode (defaults from CSS :root)
            root.style.setProperty('--navy', '#0a192f');
            root.style.setProperty('--navy-light', '#112240');
            root.style.setProperty('--navy-lighter', '#233554');
            root.style.setProperty('--white', '#e6f1ff');
            root.style.setProperty('--glass-bg', 'rgba(17, 34, 64, 0.8)');
            root.style.setProperty('--glass-border', 'rgba(255, 215, 0, 0.2)');
            root.style.setProperty('--yellow', '#ffd700');
            root.style.setProperty('--yellow-dark', '#ffb700');
        }
    }
